...
```
Error: ENOENT: no such file or directory, stat 'C:\Users\Deepak\Desktop\projects\playwright_automation_framework\Playwright_Switch2.0\UsersDeepakDesktopprojectsplaywright_automation_frameworkPlaywright_Switch2.0	ests
aukariesumeQA_DeepakJ_Resume.pdf'
```
...
>  8 |   test(`update resume on naukari for account ${cred.accName}`, async ({ page }) => 
  ...
  35 |     const filePath = path.resolve('C:\Users\Deepak\Desktop\projects\playwright_automation_framework\Playwright_Switch2.0\tests\naukari\resume\QA_DeepakJ_Resume.pdf');
  36 |
  37 |     // Listen for file chooser event
  38 |     const [fileChooser] = await Promise.all([
  39 |       page.waitForEvent('filechooser'),
  40 |       page.click('input[value="Update resume"]'), // Triggers the file chooser
  41 |     ]);
  42 |
  43 |       // Set the PDF file to the file chooser
  44 |     await fileChooser.setFiles(filePath);
  45 |
  46 |     // update resume
  47 |     //await page.locator('.dummyUpload.typ-14Bold').setInputFiles('tests/naukari/resume/QA_DeepakJ_Resume.pdf')
  48 |     
  49 |     // const fileInput = page.locator('input[type="file"]');
  50 |     // await fileInput.setInputFiles(filePath);
  51 |
  52 |     //verify resume uploaded.
  53 |     const message = page.locator('text=Resume has been successfully uploaded.');
  54 |     await expect(message).toBeVisible();
  55 |
  56 |     // navigate to profile
  57 |     await page.locator("img[alt='naukri user profile img']").click();
  58 |     //await page.waitForLoadState('networkidle');
  59 |
  60 |     await expect(page.getByText('Logout')).toBeVisible();
  61 |     // logout
  62 |     await page.locator('text=Logout').click();
  63 |     // await page.waitForLoadState('networkidle');
  64 |     // verify logout with url
  65 |     expect(page.url()).toContain("https://www.naukri.com/");
  66 |
  67 |   });
  68 |
  69 | }
